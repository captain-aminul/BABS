...
class RegionExtractor():
    ...
    '''def extract_regions(self, index):
        ...
        regions = np.zeros((len(index) * len(self.image), self.crop_size, self.crop_size, 3), dtype='uint8')
        for i, sample in enumerate(self.samples[index]):
            for j, img in enumerate(self.image):
                regions[i*len(self.image) + j] = crop_image2(img, sample, self.crop_size, self.padding)
        regions = regions.transpose(0, 3, 1, 2)
        regions = regions.astype('float32') - 128.
        return regions'''
