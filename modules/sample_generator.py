...
class SampleGenerator():
    def __init__(self, type_, img_size, trans=1, scale=1, aspect=None, valid=False):
        ...
        self.aspect = aspect
        self.valid = valid
        # print ('self.type = ',self.type,' , self.img_size = ',self.img_size,' , self.trans = ',self.trans)
        # print ('self.scale = ',self.scale, ' , self.aspect = ',self.aspect, ' , self.valid = ',self.valid)
    ...
    def _gen_samples(self, bb, n):
        ...
        # adjust bbox range
        samples[:, 2:] = np.clip(samples[:, 2:], 6, self.img_size - 6)
        if self.valid:
    ...
    def __call__(self, bbox, n, overlap_range=None, scale_range=None):
        ...
        # print ('bbox = ',bbox)
        # print ('overlap_range = ',overlap_range)
